...
void monomer:: compute_local_density_of_state(vector<vector<vector<int>>> & coupling_state_index_list,
                                              vector<vector<vector<vector<int>>>> & coupling_state_qn_list,
                                              vector<vector<vector<double>>> & coupling_state_strength_list,
                                              vector<vector<vector<double>>> & coupling_state_energy_diff_list,
                                              vector<vector<double>> & effective_coupling_number_list){
...
void monomer:: compute_local_density_of_state_subroutine(int monomer_index, vector<double> & state_energy_global_matrix,
    ...
    state_index_in_global_matrix = monomer_matsize_offset_each_process[monomer_index][state_proc_id] + state_index_in_proc;


    double energy_diff;
    int coupling_state_index;
    vector<vector<int>> & vibration_qn = monomer_vibrational_states_all[monomer_index];
    ...
    if (my_id == state_proc_id){
        ...
        for( i = monomer_matsize[monomer_index] ; i < monomer_matnum[monomer_index]; i++){
            if(monomer_irow[monomer_index][i] == state_index_in_global_matrix){
                energy_diff = abs(state_energy_global_matrix[state_index_in_global_matrix] - state_energy_global_matrix[monomer_icol[monomer_index][i]] );

                // do not include state whose energy difference is small (polyad).
//                if( abs(energy_diff) <= 10 ){
//                    continue;
//                }
                effective_coupling_number_between_states = 1 / (1 + pow(energy_diff / monomer_mat[monomer_index][i] , 2));
                effective_coupling_number = effective_coupling_number + effective_coupling_number_between_states ;
                coupling_state_index = monomer_icol[monomer_index][i] ;

                if (effective_coupling_number_between_states > 0.01 ){
                    coupling_state_index_list.push_back(coupling_state_index );
                    coupling_state_energy_diff.push_back(energy_diff);
                    coupling_state_strength.push_back(monomer_mat[monomer_index][i]);
                }
